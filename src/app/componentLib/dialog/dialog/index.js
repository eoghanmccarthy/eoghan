import React, { Fragment, forwardRef } from 'react';
import { bool, string, func } from 'prop-types';
import { useTransition, animated, config } from 'react-spring';
import FocusLock from 'react-focus-lock';
import cx from 'classnames';
import './styles.scss';

import baseProps from 'componentLib/baseProps';
import dialogBase from '../dialogBase';
import DialogBackground from '../dialogBackground';

const Dialog = forwardRef(
  (
    { children, className, isVisible, toggleContentOverlay, closeDialog, onDestroy, ...rest },
    forwardedRef
  ) => {
    const transition = useTransition(isVisible, null, {
      from: { opacity: 0, transform: 'translateY(+70px)' },
      enter: { opacity: 1, transform: 'translateY(0px)' },
      leave: { opacity: 0, transform: 'translateY(+50px)' },
      config: (item, state) =>
        state === 'leave' ? { mass: 1, tension: 380, friction: 40, clamp: true } : config.stiff,
      onDestroyed: () => !isVisible && toggleContentOverlay(false)
    });

    return transition.map(({ item, key, props }) =>
      item ? (
        <Fragment key={key}>
          <FocusLock returnFocus>
            <DialogBackground closeDialog={closeDialog}>
              <animated.div
                ref={forwardedRef}
                tabIndex={'-1'}
                style={props}
                className={cx('dialog', className)}
                onClick={e => e.stopPropagation()}
                {...rest}
              >
                {children}
              </animated.div>
            </DialogBackground>
          </FocusLock>
        </Fragment>
      ) : null
    );
  }
);

export default dialogBase(Dialog);

Dialog.propTypes = {
  toggleContentOverlay: func.isRequired
};

const DialogContent = baseProps(props => {
  const { children, className, ...rest } = props;

  return (
    <div className={cx('dialog-content', className)} {...rest}>
      {children}
    </div>
  );
});

DialogContent.propTypes = {
  id: string,
  className: string
};

export { DialogContent };
